...
def returnBook():  
    ...
    try:                   #checking burrow txt file
        ...
        with open(a,"r") as f:
            data=f.read()
            print(data)              #opening and displaying burrow txt file
    except:
        print("The borrower name is incorrect")
        returnBook()

    b="Return-"+name+".txt"
    with open(b,"w+")as f:                # creating return txt file
    ...
    for i in range(3):
        if ListSplit.bookname[i] in data:
            with open(b,"a") as f:                   #writting in return txt file
                f.write(str(i+1)+"\t\t"+ListSplit.bookname[i]+"\t\t$"+ListSplit.cost[i]+"\n")
                ListSplit.quantity[i]=int(ListSplit.quantity[i])+1                 #increasing quantity in stock
    ...
    if(stat.upper()=="Y"):
        ...
        day=int(input())
        fine=2*day                         #adding fine 
        with open(b,"a")as f:
            f.write("\t\t\t\t\tFine: $"+ str(fine)+"\n")    #adding fine in return txt file      
        total=total+fine
    


    print("Final Total: "+ "$"+str(total))                        #displaying total price 
    with open(b,"a")as f:
        f.write("\t\t\t\t\tTotal: $"+ str(total))
    
        
    with open("Stock.txt","w+") as f:
            for i in range(3):                                 #updating stock
